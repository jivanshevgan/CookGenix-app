...
export function AuthScreen({ onAuthSuccess, isDarkMode }: AuthScreenProps) {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      setError("Arre! Google login failed. Please try again.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessage(`Badiya! A reset link has been sent to ${email}. Check your Email app's Inbox and SPAM/PROMOTIONS folders. Note: Links can sometimes take 2-5 minutes to arrive.`);
    } catch (err: any) {
      console.error("Reset error:", err);
      let msg = err.message;
      if (err.code === "auth/user-not-found") msg = "No account found with this email. Check for typos!";
        ...
        <div className={`glass p-8 rounded-[40px] shadow-2xl border border-white/10 ${isDarkMode ? 'bg-white/5' : 'bg-white/80'}`}>
          ...
          <AnimatePresence mode="wait">
            {mode === "forgot" && (
              <motion.div
                ...
                className="mb-8 p-6 bg-primary/10 rounded-3xl border border-primary/20 text-center"
              >
                <h3 className="text-lg font-black mb-2">Forgot Password?</h3>
                <p className="text-[10px] font-bold opacity-60 uppercase tracking-widest leading-relaxed">
                  Enter your email below and we'll send you a magic link to reset it.
                </p>
              </motion.div>
            )}
          </AnimatePresence>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            >
              {isLoading ? (
                ...
                <>
                  {mode === 'login' ? 'Let\'s Cook' : mode === 'signup' ? 'Create Account' : 'Send Reset Link'}
          ...
          <div className="mt-8 flex flex-col items-center gap-6">
            <div className="flex items-center gap-4 w-full">
              ...
              <p className="text-[10px] font-black uppercase tracking-widest opacity-30">OR</p>
              <div className="h-[1px] flex-1 bg-black/10 dark:bg-white/10" />
            </div>
            ...
            <button 
              ...
              disabled={isLoading}
              className="w-full py-4 bg-white dark:bg-white/10 text-gray-900 dark:text-white border border-black/10 dark:border-white/10 font-bold rounded-2xl shadow-sm hover:shadow-lg hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-3 disabled:opacity-50"
            >
